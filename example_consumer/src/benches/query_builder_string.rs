    ...
    let result = Neo4gBuilder::new()
        .get()
            ...
            .filter(Where::new()
                ...
                .nest(|inner| {
                    inner.condition(&component1, Some(&ComponentProps::Id("asdfasdf".into())), CompareOperator::Ne("asdfvxcvxcv".into()))
                    .join(CompareJoiner::And)
                    .condition(&component2, Some(&ComponentProps::Id("asdfasdfasdf".into())), CompareOperator::Ne("v=casdsaerg".into()))
                })
            )
        .end_statement()
        .build();
